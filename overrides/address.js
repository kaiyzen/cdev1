...
const { sha3_256, keccak256 } = require('js-sha3');
...
const address = {
	...
	publicKeyToAddress: (publicKey, networkIdentifier) => {
		...
		const publicKeyHash = keccak256.arrayBuffer(publicKey);
		...
		const hash = keccak256.arrayBuffer(decodedAddress.subarray(0, constants.sizes.ripemd160 + 1));
